...
const PersonaAvatar = ({ persona, size = 56 }) => {
  ...
  const isB2B = p.includes('b2b') || p.includes('internal');
  if (isB2B) {
    return (
      <div style={{ width: size, height: size, borderRadius: '50%', background: 'rgba(26,77,94,0.08)', border: '1px solid rgba(26,77,94,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
        <svg width={size * 0.55} height={size * 0.55} viewBox="0 0 32 32" fill="none" stroke="#3E9DB5" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round">
          <rect x="4" y="10" width="24" height="18" /><path d="M12 10 V6 H20 V10" /><line x1="4" y1="18" x2="28" y2="18" />
        </svg>
      </div>
    );
  }
  const isMale = /^\s*m\b/i.test(persona || '');
  let h = 0; for (let i = 0; i < (persona || '').length; i++) h = (h * 31 + persona.charCodeAt(i)) >>> 0;
  const femaleFaces = ['face-a', 'face-b', 'face-c', 'face-d'];
  const face = isMale ? 'face-e' : femaleFaces[h % femaleFaces.length];
  return (
    <div style={{ width: size, height: size, borderRadius: '50%', overflow: 'hidden', border: '1px solid rgba(62,157,181,0.3)', background: '#10161E', flexShrink: 0 }}>
      <img src={'/' + face + '.png'} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', objectPosition: '50% 12%', display: 'block' }} />
    </div>
  );
};
...
function LaunchPhase({ launch, setLaunch, onComplete, setupCash }) {
  ...
  return (
    <div>
      ...
      <Primary onClick={() => { sfx.confirm(); onComplete(); }} disabled={!valid}>Open the Clinic →</Primary>
    </div>
  );
}
...
function FirstPatientConsult({ onComplete, regulator }) {
  ...
  return (
    <div>
      <div className="ai-fade-in" style={{ background: 'linear-gradient(135deg, rgba(103,184,107,0.16), rgba(103,184,107,0.04))', border: '1px solid rgba(103,184,107,0.4)', borderLeft: '3px solid #67B86B', borderRadius: 9, padding: '11px 14px', marginBottom: 14, display: 'flex', alignItems: 'center', gap: 11 }}>
        <div className="ai-streak-glow" style={{ width: 34, height: 34, borderRadius: '50%', background: 'rgba(103,184,107,0.18)', border: '1px solid rgba(103,184,107,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
          <svg width="17" height="17" viewBox="0 0 24 24"><path d="M5 13 l4 4 L19 7" fill="none" stroke="#67B86B" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" /></svg>
        </div>
        <div>
          <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 9.5, letterSpacing: '0.2em', textTransform: 'uppercase', color: '#67B86B', fontWeight: 700 }}>Your Clinic Is Open</div>
          <div style={{ fontFamily: "'IBM Plex Sans', system-ui, sans-serif", fontSize: 14, fontWeight: 500, color: '#E8EDF2', marginTop: 2 }}>Doors unlocked. Here comes your first booking.</div>
        </div>
      </div>
      ...
      <div style={{ minHeight: '100vh', background: 'linear-gradient(180deg, #161F28 0%, #0E141A 100%)', color: '#E8EDF2', fontFamily: "'IBM Plex Sans', system-ui, sans-serif", padding: '22px 18px 80px' }}>
        ...
        <div style={{ maxWidth: 600, margin: '0 auto' }}>
          ...
          {phase === 'setup' && (() => {
            ...
            const figure = (o) => {
              const b = [];
              if (o.cost !== undefined && o.cost > 0) b.push('−£' + (o.cost < 1 ? Math.round(o.cost * 1000) : o.cost + 'k'));
              if (o.cash !== undefined && o.cash > 0) b.push('+£' + o.cash + 'k');
              if (o.equity !== undefined && o.equity > 0) b.push('−' + o.equity + '% equity');
              return b.join(' · ');
            };
            const CardPick = ({ value, onChange, options, cols = 1, disabled }) => (
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(' + cols + ',1fr)', gap: 6, marginBottom: 6 }}>
                {options.map(o => {
                  const sel = value === o.id;
                  const fig = figure(o);
                  return (
                    <button key={o.id} onClick={() => { if (!disabled) { sfx.select(); onChange(o.id); } }} disabled={disabled && !sel} style={{
                      background: sel ? '#222F3A' : 'rgba(255,255,255,0.04)',
                      border: '1px solid ' + (sel ? '#3E9DB5' : 'rgba(255,255,255,0.1)'),
                      borderRadius: 9, padding: '11px 12px', textAlign: 'left',
                      cursor: disabled ? 'not-allowed' : 'pointer', opacity: disabled && !sel ? 0.4 : 1,
                      fontFamily: 'inherit', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8,
                      transition: 'all 0.16s',
                    }}>
                      <span style={{ fontFamily: "'IBM Plex Sans', system-ui, sans-serif", fontSize: 13.5, fontWeight: 500, color: '#E8EDF2', lineHeight: 1.25 }}>{o.label}</span>
                      {fig && <span style={{ fontFamily: "'IBM Plex Sans', system-ui, sans-serif", fontSize: 11, color: '#37AEC8', fontWeight: 500, flexShrink: 0, whiteSpace: 'nowrap' }}>{fig}</span>}
                    </button>
                  );
                })}
              </div>
            ...
            return (
              <div>
                ...
                {/* Live "your clinic so far" summary — builds as choices are made */}
                {(() => {
                  const bgO = BACKGROUNDS.find(b => b.id === setup.background);
                  const trO = TRAINING_PATHS.find(t => t.id === setup.training);
                  const stO = STRUCTURES.find(s => s.id === setup.structure);
                  const locO = LOCATIONS.find(l => l.id === setup.location);
                  const prO = PREMISES.find(p => p.id === setup.premises);
                  const chips = [
                    bgO && { k: 'Role', v: bgO.label },
                    trO && { k: 'Trained', v: trO.label },
                    stO && { k: 'Structure', v: stO.label },
                    locO && { k: 'Location', v: locO.shortLabel },
                    prO && { k: 'Premises', v: prO.shortLabel || prO.label },
                  ].filter(Boolean);
                      ...
                      <div style={{ flex: 1, display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                        {chips.map((c, i) => (
                          <div key={i} style={{ display: 'flex', alignItems: 'baseline', gap: 5, background: 'rgba(255,255,255,0.05)', borderRadius: 6, padding: '4px 9px' }}>
                            <span style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 8.5, letterSpacing: '0.08em', textTransform: 'uppercase', color: '#6F7B89' }}>{c.k}</span>
                            <span style={{ fontFamily: "'IBM Plex Sans', system-ui, sans-serif", fontSize: 12, fontWeight: 500, color: '#E8EDF2' }}>{c.v}</span>
                          </div>
                        ))}
                      </div>
                    </div>
                ...
                {setupStep === 1 && (
                  <div className="ai-fade-in">
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6, marginBottom: 16 }}>
                      {BACKGROUNDS.map(b => (
                        <button key={b.id} onClick={() => { if (!isRestarting) { sfx.select(); setSetup({ ...setup, background: b.id }); } }} disabled={isRestarting} style={{
                    ...
                    <CardPick value={setup.training} onChange={(v) => !isRestarting && setSetup({ ...setup, training: v })} options={TRAINING_PATHS} cols={2} disabled={isRestarting} />
                    ...
                    <CardPick value={setup.structure} onChange={(v) => setSetup({ ...setup, structure: v })} options={STRUCTURES} cols={2} />
                ...
                {setupStep === 2 && (
                  <div className="ai-fade-in">
                    ...
                    <CardPick value={setup.premises} onChange={(v) => setSetup({ ...setup, premises: v })} cols={2} options={PREMISES.map(p => {
                      const loc = LOCATIONS.find(l => l.id === setup.location);
                    ...
                    <div style={{ display: 'grid', gap: 8, marginBottom: 10 }}>
                      <div>
                        ...
                        <CardPick value={setup.toxin} onChange={(v) => setSetup({ ...setup, toxin: v })} options={TOXINS} cols={2} />
                      </div>
                      <div>
                        ...
                        <CardPick value={setup.filler} onChange={(v) => setSetup({ ...setup, filler: v })} options={FILLERS} cols={2} />
                      </div>
                      <div>
                        ...
                        <CardPick value={setup.biostim} onChange={(v) => setSetup({ ...setup, biostim: v })} options={BIOSTIMS} cols={2} />
                      </div>
                    </div>
